...
def check_states(option: str) -> None:

    """
    Check the Pokémon state transitions either using a capture card or a video file.

    Args:
        option (str): '1' to use the capture card, '2' to use a video file.
    ...
    # Initialize the video capture source depending on the option
    if option == '1':
    ...
    while True:
        ...
        # Pause/resume with 'spacebar'
            ...
            pause = not pause
        # Move one frame left when paused with 'a', 'A' or 'left arrow'
        elif pause and key in [ord('a'), ord('A'), LEFT_ARROW_KEY_VALUE]:
            current_frames = max(0, current_frames - 2)
            force_load_frame = True
        # Move one frame right when paused with 'd', 'D' or 'right arrow'
        elif pause and key in [ord('d'), ord('D'), RIGHT_ARROW_KEY_VALUE]:
            force_load_frame = True

        if pause and not force_load_frame:
            continue

        # Load a specific frame if a key was pressed
        if force_load_frame:
            Video_Capture.video_capture.set(cv2.CAP_PROP_POS_FRAMES, current_frames)

        # Wait for 0.05s if using video file to simulate ~20 FPS
